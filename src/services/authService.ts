...
export const login = async (email: string, password: string): Promise<{ user?: User; error?: string }> => {
  try {
    ...
    return { error: error?.message || 'Đăng nhập thất bại. Vui lòng kiểm tra lại email và mật khẩu.' };
  }
};
...
export const register = async (
  ...
  try {
    ...
    return { error: error?.message || 'Đăng ký thất bại. Email có thể đã được sử dụng.' };
  }
};
